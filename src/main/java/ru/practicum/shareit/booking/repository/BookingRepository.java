...
public interface BookingRepository extends JpaRepository<Booking, Long> {
    ...
    @Query("SELECT b FROM Booking b " + " WHERE b.booker.id = :bookerId " + "   AND b.start <= :now " + "   AND b.end >= :now " + "   AND b.status = ru.practicum.shareit.booking.model.Status.APPROVED")
    Page<Booking> findCurrentByBooker(@Param("bookerId") long bookerId, @Param("now") LocalDateTime now, Pageable pageable);

    @Query("SELECT b FROM Booking b " + " WHERE b.item.owner.id = :ownerId " + "   AND b.start <= :now " + "   AND b.end >= :now " + "   AND b.status = ru.practicum.shareit.booking.model.Status.APPROVED")
    Page<Booking> findCurrentByOwner(@Param("ownerId") long ownerId, @Param("now") LocalDateTime now, Pageable pageable);
